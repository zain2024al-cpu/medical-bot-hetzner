...
#   4.  عدد المستلزمات    — free text input, رقم أو وصف (STEP_COUNT)
...
async def _handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── عدد المستلزمات — نص حر (رقم أو وصف المستلزمات)؛ يُرفَض الفارغ فقط ──
...
async def _handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not query.from_user or not _is_authorized(query.from_user.id):
        logger.warning(f"[supplies] 🚫 blocked unauthorized user={getattr(query.from_user, 'id', '?')}")
        return
