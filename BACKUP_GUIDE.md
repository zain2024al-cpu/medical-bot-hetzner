...
| 4 | نسخ بالبريد أسبوعي | أسبوعياً | **المشروع كاملاً**: الكود + `.env`/`config.env` + قاعدة البيانات | بريدك الإلكتروني |
...
- الطبقة 4 (بريد أسبوعي): نسخة **خارج السيرفر** لكل شيء — الكود *و* الإعدادات السرية (`BOT_TOKEN`, `ADMIN_IDS`, إلخ) اللي **لا توجد في GitHub أصلاً** (لأسباب أمنية) ولا في أي نسخة أخرى. لو ضاع السيرفر بالكامل، هذه هي الطريقة الوحيدة لاسترجاع ملفات الإعدادات بدون إعادة كتابتها يدوياً من الذاكرة.
...
## الطبقة 4: النسخ بالبريد الأسبوعي (المشروع كاملاً)

- **السكربت**: `/home/botuser/full_project_backup.sh` (عبر cron، أسبوعياً — الأحد 4 صباحاً)
- **المحتوى**: أرشيف `tar.gz` يشمل:
  - كل كود المشروع (بدون `venv/` — يُعاد تثبيته من `requirements.txt` عند الحاجة، وبدون `.git/` و`__pycache__` و`db/backups/` لتقليل الحجم)
...
```bash
# فك ضغط الأرشيف الأسبوعي المُستلَم بالبريد
tar -xzf project_full_YYYYMMDD_HHMMSS.tar.gz -C ~/

# سيحتوي على مجلد medical-bot-hetzner كامل بما فيه .env و config.env
