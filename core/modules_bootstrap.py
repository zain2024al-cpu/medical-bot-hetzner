...
def bootstrap_all() -> None:
    ...
    registry.register(
        ...
        # ⚠️ «🧾 طباعة الفواتير» (جدول الأرقام) **مخفيّ بطلب المستخدم** —
        # الشيفرة باقية كاملةً (`start_pharmacy_invoices` +
        # `services/pharmacy_invoices_pdf.py`). لإعادته: أضف نصّه هنا في
        # `menu_buttons` و`keyboard_rows`، وأعِد سطر `MessageHandler` في
        # `pharmacy_print/flow.py` وزرّه في `admin_system_menu.py`.
        menu_buttons={"🖨️ طباعة مسير الإخلاء", "📸 صور الفواتير"},
        keyboard_rows=[["🖨️ طباعة مسير الإخلاء", "📸 صور الفواتير"]],
