...
# ⚠️ لا خط عربي متاح على الخادم يملك محارف الإيموجي (Tahoma/Arial/
# NotoNaskh/الخط المرفَق) — فكل إيموجي يُطبَع **مربعاً فارغاً □**. هذا ما
# جعل ملف الإقامة يبدو "غلطاً كبيراً": مربع قبل كل عنوان وفي كل خلية حالة.
# تُحذَف هنا مرة واحدة لكل مولّدات PDF بدل مطاردتها نصّاً نصّاً (سبق أن
# اضطرّ تقرير التقييم لحذفها يدوياً من تسمياته).
...
def strip_emoji(text: str) -> str:
    """يحذف الإيموجي ويكنس المسافات المزدوجة الناتجة عنه."""
    out = _EMOJI_RE.sub("", text)
    # "🪪 صورة الإقامة:" ⇒ " صورة الإقامة:" ⇒ "صورة الإقامة:"
    while "  " in out:
        out = out.replace("  ", " ")
    return out.strip()
