...
def send_backup_via_email(backup_path: str) -> bool:
    """يضغط ملف النسخة الاحتياطية ويرسله بالبريد. يعيد True عند النجاح
    فقط — أي فشل (بيانات ناقصة، شبكة، SMTP) يُسجَّل ويُعاد False بلا رفع
    أي استثناء للمستدعي."""
    ...
    try:
        ...
        msg["From"] = from_addr
        msg["To"] = to_addr
        msg["Subject"] = f"نسخة احتياطية يومية — {os.path.basename(backup_path)}"
        msg.attach(MIMEText(
        ...
        logger.info(f"✅ [backup_email] أُرسلت النسخة الاحتياطية بالبريد إلى {to_addr}")
        return True

    except Exception as exc:
        logger.error(f"❌ [backup_email] فشل إرسال النسخة الاحتياطية بالبريد: {exc}", exc_info=True)
        return False
