...
# 🔧 معرف مجموعة الخدمات العامة (نشر تقارير المغادرة والخدمات)
GENERAL_SERVICES_GROUP_ID = os.getenv("GENERAL_SERVICES_GROUP_ID", "")

# 🛬 مجموعة **تقارير الوصول** وحدها — مفصولة عن بقية الخدمات العامة.
#
# ⚠️ **متغيّر باسم جديد عمداً**: `GENERAL_SERVICES_GROUP_ID` مضبوط في
# `.env` على الخادم بمجموعة أخرى، فلو أُعيد استعماله لعمل الإعداد القديم
# فوق الجديد بصمت — نفس فخّ `RESIDENCY_GROUP_ID` الذي مرّ بنا.
#
# ⚠️ و`or` لا الوسيط الثاني وحده: `getenv` يُرجِع القيمة الفارغة إن كان
# المتغيّر موجوداً وفارغاً في `.env`، فيتغلّب الفراغ على الافتراضي. فالقيمة
# هنا **لا تكون فارغة أبداً** — وللتحويل إلى مجموعة أخرى اضبطها صراحةً.
GS_ARRIVALS_GROUP_ID = (
    os.getenv("GS_ARRIVALS_GROUP_ID", "").strip() or "-1002381061606"
)

# 🪪 معرف مجموعة الإقامات (إعداد قديم — لا يستعمله شيء حالياً)
RESIDENCY_GROUP_ID = os.getenv("RESIDENCY_GROUP_ID", "")

# 🪪 مجموعة نشر **ملفات الإقامات الجديدة** عند توثيقها.
#
# ⚠️ **متغيّر مستقلّ عن `RESIDENCY_GROUP_ID` عمداً**: ذاك مضبوط في
# `.env` على الخادم بمجموعة أخرى قديمة (`-1004420231283`) ولا يستعمله
# شيء — فلو قرأ النشر منه لذهبت الملفات إلى مجموعة غير المقصودة، بلا
# خطأ ولا تحذير، وأول من يكتشفها فريقٌ لم يصله شيء.
#
# ⚠️ و`or` لا الوسيط الثاني وحده: `getenv` يُرجِع القيمة الفارغة إن كان
# المتغيّر موجوداً وفارغاً، فيتغلّب الفراغ على الافتراضي ويتعطّل النشر.
RESIDENCY_ISSUANCE_GROUP_ID = (
    os.getenv("RESIDENCY_ISSUANCE_GROUP_ID", "").strip() or "-1002157820401"
)

# 🔕 إيقاف إشعار الأدمن الخاص لتقارير الخدمات العامة والإقامات.
# الافتراضي "1" = مُفعَّل (السلوك الأصلي بلا تغيير). اضبطه على 0 أثناء
# الاختبار فتذهب التقارير إلى المجموعة وحدها بلا إزعاج كل أدمن برسالة خاصة.
# ⚠️ يمسّ ناشرَي general_services وresidency فقط — لا يمسّ تقارير المترجمين
# ولا الرعاية الصحية، فلها مسار نشر مستقل.
GS_NOTIFY_ADMINS = os.getenv("GS_NOTIFY_ADMINS", "1").strip().lower() not in (
    "0", "false", "no", "off", ""
)
