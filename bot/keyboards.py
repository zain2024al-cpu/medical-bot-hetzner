...
def admin_main_kb():
    ...
    keyboard = [
        ...
        # ✅ زر مسير الصيدلية أُعيد للأدمن بطلب صريح: يرى الأدمن كل السجلات
        # (بلا عزل — is_admin=True يُسقط شرط العزل في
        # get_evacuation_ledger_rows تلقائياً، لا حاجة لأي تعديل هناك).
        # لا علاقة لهذا بصلاحية "pharmacy_print" في قاعدة البيانات؛
        # is_admin() في _is_authorized يكفي وحده لتخطّي ذلك الفحص.
        ["🖨️ طباعة مسير الإخلاء"],
        # ملاحظة: "💰 التقرير المالي" ما زال مُزالاً من واجهة الأدمن (بناءً
        # على طلب صريح سابق) — الأدمن لإدارة الصلاحيات لا لاستخدامه. الميزة
        # كاملة ولم تُمَس، ويظهر للمستخدمين المخوَّلين عبر dynamic_user_kb().
