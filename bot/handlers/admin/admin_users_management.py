...
# ðŸ‘¥ Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† - Ù†Ø³Ø®Ø© Ù…Ø¨Ø³Ø·Ø©
# =============================
...
async def start_user_management(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"ðŸ” Ù…Ø­Ø§ÙˆÙ„Ø© Ø§Ù„ÙˆØµÙˆÙ„ Ù„Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ù…Ù†: {user.id}")
    
    if not is_admin(user.id):
        logger.warning(f"âŒ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… {user.id} Ù„ÙŠØ³ Ø£Ø¯Ù…Ù†")
        await update.message.reply_text("ðŸš« Ù‡Ø°Ù‡ Ø§Ù„Ø®Ø§ØµÙŠØ© Ù…Ø®ØµØµØ© Ù„Ù„Ø¥Ø¯Ù…Ù† ÙÙ‚Ø·.")
...
def _main_kb():
    """Ù„ÙˆØ­Ø© Ø§Ù„Ù…ÙØ§ØªÙŠØ­ Ø§Ù„Ø±Ø¦ÙŠØ³ÙŠØ©"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("ðŸ“‹ Ø¹Ø±Ø¶ Ø¬Ù…ÙŠØ¹ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†", callback_data="um:view_all")],
        ...
        [InlineKeyboardButton("ðŸ”’ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ø§Ù„Ù…Ø¬Ù…Ø¯ÙŠÙ†", callback_data="um:view_suspended")],
        [InlineKeyboardButton("ðŸ” Ø¨Ø­Ø« Ø¹Ù† Ù…Ø³ØªØ®Ø¯Ù…", callback_data="um:search")],
        [InlineKeyboardButton("ðŸ“Š Ø¥Ø­ØµØ§Ø¦ÙŠØ§Øª", callback_data="um:stats")],
        [InlineKeyboardButton("âŒ Ø¥Ù„ØºØ§Ø¡", callback_data="um:cancel")]
    ])

def _users_kb(users, user_type="all"):
    """Ù„ÙˆØ­Ø© Ù…ÙØ§ØªÙŠØ­ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†"""
    buttons = []
    ...
    buttons.append([InlineKeyboardButton("ðŸ”™ Ø±Ø¬ÙˆØ¹", callback_data="um:back")])
    return InlineKeyboardMarkup(buttons)

def _user_actions_kb():
    """Ù„ÙˆØ­Ø© Ù…ÙØ§ØªÙŠØ­ Ø¥Ø¬Ø±Ø§Ø¡Ø§Øª Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…"""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("âœ… Ù…ÙˆØ§ÙÙ‚Ø©", callback_data="um:approve")],
        [InlineKeyboardButton("âŒ Ø±ÙØ¶", callback_data="um:reject")],
        [InlineKeyboardButton("ðŸ”’ ØªØ¬Ù…ÙŠØ¯", callback_data="um:suspend")],
        [InlineKeyboardButton("ðŸ”“ Ø¥Ù„ØºØ§Ø¡ ØªØ¬Ù…ÙŠØ¯", callback_data="um:unsuspend")],
        [InlineKeyboardButton("ðŸš« Ø¥Ø®Ø±Ø§Ø¬ Ù…Ù† Ø§Ù„Ø¨ÙˆØª", callback_data="um:delete")],
        [InlineKeyboardButton("ðŸ”™ Ø±Ø¬ÙˆØ¹", callback_data="um:back")]
    ])
...
async def handle_user_management_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ø¥Ø²Ø§Ù„Ø© Ø§Ù„Ø¨Ø§Ø¯Ø¦Ø© "um:" Ù„Ù„Ø­ØµÙˆÙ„ Ø¹Ù„Ù‰ Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„ÙØ¹Ù„ÙŠØ©
    ...
    try:
        if data == "view_all":
            ...
            await query.edit_message_text("âŒ ØªÙ… Ø¥Ù„ØºØ§Ø¡ Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†.")
            return ConversationHandler.END
    except Exception as e:
        logger.error(f"Error in user management: {e}", exc_info=True)
        await query.edit_message_text("âŒ Ø­Ø¯Ø« Ø®Ø·Ø£. ÙŠØ±Ø¬Ù‰ Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø© Ù…Ø±Ø© Ø£Ø®Ø±Ù‰.")
        return UM_START
    ...
    with SessionLocal() as s:
        ...
        if not users:
            await query.message.reply_text(
                "ðŸ“‹ **Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ù…Ø³Ø¬Ù„ÙŠÙ†**\n\n"
        ...
        text = f"ðŸ“‹ **Ø¬Ù…ÙŠØ¹ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† ({len(users)})**\n\n"
        text += "Ø§Ø®ØªØ± Ù…Ø³ØªØ®Ø¯Ù…Ø§Ù‹ Ù„Ø¹Ø±Ø¶ Ø§Ù„ØªÙØ§ØµÙŠÙ„:"
    ...
    with SessionLocal() as s:
        ...
        if not users:
            ...
            await query.message.reply_text(
                ...
                "âœ… Ù„Ø§ ØªÙˆØ¬Ø¯ Ø·Ù„Ø¨Ø§Øª Ø§Ù†Ø¶Ù…Ø§Ù… Ø¨Ø§Ù†ØªØ¸Ø§Ø± Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø©.\n\n"
                "Ø§Ø®ØªØ± Ø®ÙŠØ§Ø±Ø§Ù‹ Ø¢Ø®Ø±:",
                reply_markup=_main_kb(),
                parse_mode="Markdown"
            )
...
async def _show_approved_users(query, context):
    """Ø¹Ø±Ø¶ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ø§Ù„Ù…ÙˆØ§ÙÙ‚ Ø¹Ù„ÙŠÙ‡Ù…"""
    with SessionLocal() as s:
        ...
        if not users:
            await query.message.reply_text(
                ...
                "Ù„Ù… ÙŠØªÙ… Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø¹Ù„Ù‰ Ø£ÙŠ Ù…Ø³ØªØ®Ø¯Ù… Ø¨Ø¹Ø¯.\n\n"
                "Ø§Ø®ØªØ± Ø®ÙŠØ§Ø±Ø§Ù‹ Ø¢Ø®Ø±:",
                reply_markup=_main_kb(),
                parse_mode="Markdown"
            )
...
async def _show_user_details(query, context, user_id):
    """Ø¹Ø±Ø¶ ØªÙØ§ØµÙŠÙ„ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…"""
    ...
    with SessionLocal() as s:
        ...
        if not user:
            await query.edit_message_text("âŒ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯.", reply_markup=_main_kb())
            return UM_START
        
        context.user_data["selected_user_id"] = user_id
        
        # Ø­Ø³Ø§Ø¨ Ø¹Ø¯Ø¯ Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ± (Ø¨Ø§Ø³ØªØ®Ø¯Ø§Ù… func.count Ù„ØªØ¬Ù†Ø¨ Ù…Ø´Ø§ÙƒÙ„ Ø§Ù„Ø£Ø¹Ù…Ø¯Ø© Ø§Ù„Ù…ÙÙ‚ÙˆØ¯Ø©)
        try:
            reports_count = s.query(func.count(Report.id)).filter_by(translator_id=user.id).scalar() or 0
        except Exception as e:
            logger.error(f"Ø®Ø·Ø£ ÙÙŠ Ø­Ø³Ø§Ø¨ Ø¹Ø¯Ø¯ Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ±: {e}")
            reports_count = 0
        
        # Ø¢Ø®Ø± ØªÙ‚Ø±ÙŠØ± (Ø¨Ø§Ø³ØªØ®Ø¯Ø§Ù… ÙÙ‚Ø· Ø§Ù„Ø£Ø¹Ù…Ø¯Ø© Ø§Ù„Ø£Ø³Ø§Ø³ÙŠØ©)
        try:
            ...
            logger.error(f"Ø®Ø·Ø£ ÙÙŠ Ø¬Ù„Ø¨ Ø¢Ø®Ø± ØªÙ‚Ø±ÙŠØ±: {e}")
            last_activity = "Ù„Ø§ ÙŠÙˆØ¬Ø¯"
        
        status = "âœ… Ù†Ø´Ø·" if user.is_approved else "â³ Ù…Ø¹Ù„Ù‚"
        suspended = "ðŸ”’ Ù…Ø¬Ù…Ø¯" if getattr(user, 'is_suspended', False) else "ðŸ”“ Ù†Ø´Ø·"
        
        text = f"ðŸ‘¤ **ØªÙØ§ØµÙŠÙ„ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…**\n"
        text += f"â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n\n"
        text += f"ðŸ†” **Database ID:** {user.id}\n"
        text += f"ðŸ“± **Telegram ID:** `{user.tg_user_id}`\n"
        text += f"ðŸ‘¤ **Ø§Ù„Ø§Ø³Ù…:** {user.full_name}\n"
        text += f"ðŸ“ž **Ø§Ù„Ù‡Ø§ØªÙ:** {user.phone_number or 'ØºÙŠØ± Ù…Ø­Ø¯Ø¯'}\n\n"
        text += f"ðŸ“… **ØªØ§Ø±ÙŠØ® Ø§Ù„ØªØ³Ø¬ÙŠÙ„:** {user.created_at.strftime('%Y-%m-%d %H:%M')}\n"
        text += f"â° **Ø¢Ø®Ø± Ù†Ø´Ø§Ø·:** {last_activity}\n\n"
        text += f"ðŸ“Š **Ø­Ø§Ù„Ø© Ø§Ù„Ø­Ø³Ø§Ø¨:** {status}\n"
        text += f"ðŸ” **Ø­Ø§Ù„Ø© Ø§Ù„ÙˆØµÙˆÙ„:** {suspended}\n"
        text += f"ðŸ“ **Ø¹Ø¯Ø¯ Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ±:** {reports_count}\n"
        ...
        if getattr(user, 'is_suspended', False) and getattr(user, 'suspended_at', None):
            ...
            if getattr(user, 'suspension_reason', None):
                text += f"ðŸ“‹ **Ø³Ø¨Ø¨ Ø§Ù„ØªØ¬Ù…ÙŠØ¯:** {user.suspension_reason}\n"
...
async def _handle_user_action(query, context, action):
    ...
    if not user_id:
        await query.edit_message_text("âŒ Ù„Ù… ÙŠØªÙ… Ø§Ø®ØªÙŠØ§Ø± Ù…Ø³ØªØ®Ø¯Ù….", reply_markup=_main_kb())
        return UM_START
    ...
    with SessionLocal() as s:
        ...
        if action == "approve":
            ...
            s.commit()
            message = f"âœ… **ØªÙ… Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø¹Ù„Ù‰ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…**\n\nðŸ‘¤ {user_name}"
            
            # Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ù„Ù„Ù…Ø³ØªØ®Ø¯Ù…
            try:
                await context.bot.send_message(
                    chat_id=user_tg_id,
                    text="ðŸŽ‰ **Ù…Ø±Ø­Ø¨Ø§Ù‹ Ø¨Ùƒ!**\n\n"
                         "âœ… ØªÙ… Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø¹Ù„Ù‰ Ø­Ø³Ø§Ø¨Ùƒ.\n"
                         "ÙŠÙ…ÙƒÙ†Ùƒ Ø§Ù„Ø¢Ù† Ø§Ø³ØªØ®Ø¯Ø§Ù… Ø§Ù„Ù†Ø¸Ø§Ù… Ø¨Ø§Ù„ÙƒØ§Ù…Ù„.\n\n"
                         "Ø§Ø¶ØºØ· /start Ù„Ù„Ø¨Ø¯Ø¡!",
                    parse_mode="Markdown"
                )
            except Exception as e:
                logger.error(f"ÙØ´Ù„ Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø©: {e}")
                
        elif action == "reject":
            s.delete(user)
            s.commit()
            message = f"âŒ **ØªÙ… Ø±ÙØ¶ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…**\n\nðŸ‘¤ {user_name}"
            
            # Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ù„Ù„Ù…Ø³ØªØ®Ø¯Ù…
            try:
                await context.bot.send_message(
                    chat_id=user_tg_id,
                    text="âŒ **ØªÙ… Ø±ÙØ¶ Ø·Ù„Ø¨Ùƒ**\n\n"
                         "Ø¹Ø°Ø±Ø§Ù‹ØŒ Ù„Ù… ØªØªÙ… Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø¹Ù„Ù‰ Ø­Ø³Ø§Ø¨Ùƒ.\n"
                         "Ù„Ù„Ù…Ø²ÙŠØ¯ Ù…Ù† Ø§Ù„Ù…Ø¹Ù„ÙˆÙ…Ø§ØªØŒ ÙŠØ±Ø¬Ù‰ Ø§Ù„ØªÙˆØ§ØµÙ„ Ù…Ø¹ Ø§Ù„Ø¥Ø¯Ø§Ø±Ø©.",
                    parse_mode="Markdown"
                )
            except Exception as e:
                logger.error(f"ÙØ´Ù„ Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ø§Ù„Ø±ÙØ¶: {e}")
                
        elif action == "suspend":
            # Ø­ÙØ¸ Ù…Ø¹Ù„ÙˆÙ…Ø§Øª Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… Ù„Ù„Ø§Ø³ØªØ®Ø¯Ø§Ù… Ù„Ø§Ø­Ù‚Ø§Ù‹
            ...
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("âš ï¸ Ù…Ø®Ø§Ù„ÙØ© Ù„Ù„Ù†Ø¸Ø§Ù…", callback_data="suspend_reason:violation")],
                [InlineKeyboardButton("ðŸ“‰ Ø£Ø¯Ø§Ø¡ Ø¶Ø¹ÙŠÙ", callback_data="suspend_reason:performance")],
                [InlineKeyboardButton("ðŸš« Ø³Ù„ÙˆÙƒ ØºÙŠØ± Ù„Ø§Ø¦Ù‚", callback_data="suspend_reason:behavior")],
                [InlineKeyboardButton("â° ØªØ£Ø®Ø± ÙÙŠ Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ±", callback_data="suspend_reason:late")],
                [InlineKeyboardButton("âœï¸ Ø³Ø¨Ø¨ Ø¢Ø®Ø± (Ø£Ø¯Ø®Ù„ ÙŠØ¯ÙˆÙŠØ§Ù‹)", callback_data="suspend_reason:custom")],
                [InlineKeyboardButton("ðŸ”™ Ø¥Ù„ØºØ§Ø¡", callback_data="um:back")]
            ])
            
            await query.edit_message_text(
                f"ðŸ”’ **ØªØ¬Ù…ÙŠØ¯ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…: {user_name}**\n\n"
            ...
            s.commit()
            message = f"ðŸ”“ **ØªÙ… Ø¥Ù„ØºØ§Ø¡ ØªØ¬Ù…ÙŠØ¯ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… Ø¨Ù†Ø¬Ø§Ø­**\n\n"
            message += f"ðŸ‘¤ **Ø§Ù„Ø§Ø³Ù…:** {user_name}\n"
            ...
            try:
                await context.bot.send_message(
                    chat_id=user_tg_id,
                    text="ðŸ”“ **ØªÙ… ØªÙØ¹ÙŠÙ„ Ø­Ø³Ø§Ø¨Ùƒ**\n\n"
                ...
                logger.error(f"ÙØ´Ù„ Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ø¥Ù„ØºØ§Ø¡ Ø§Ù„ØªØ¬Ù…ÙŠØ¯: {e}")
            ...
            keyboard = InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("âŒ Ù„Ø§ØŒ Ø¥Ù„ØºØ§Ø¡", callback_data="confirm_delete:no")]
            ])
            
            await query.edit_message_text(
                f"âš ï¸ **ØªØ­Ø°ÙŠØ±: Ø­Ø°Ù Ù†Ù‡Ø§Ø¦ÙŠ!**\n\n"
                f"ðŸ‘¤ **Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…:** {user_name}\n\n"
                f"ðŸ—‘ Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø­Ø°Ù Ù‡Ø°Ø§ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… **Ù†Ù‡Ø§Ø¦ÙŠØ§Ù‹**ØŸ\n\n"
                f"âš ï¸ **ØªØ­Ø°ÙŠØ±Ø§Øª:**\n"
                f"âŒ Ø³ÙŠØªÙ… Ø­Ø°Ù Ø¬Ù…ÙŠØ¹ Ø¨ÙŠØ§Ù†Ø§ØªÙ‡\n"
                f"âŒ Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ù„ØªØ±Ø§Ø¬Ø¹ Ø¹Ù† Ù‡Ø°Ø§ Ø§Ù„Ø¥Ø¬Ø±Ø§Ø¡\n"
                f"âŒ Ø³ÙŠØªÙ… Ø¥Ø®Ø±Ø§Ø¬Ù‡ Ù…Ù† Ø§Ù„Ø¨ÙˆØª Ù†Ù‡Ø§Ø¦ÙŠØ§Ù‹\n\n"
                f"ðŸ’¡ **Ù†ØµÙŠØ­Ø©:** Ø¥Ø°Ø§ ÙƒÙ†Øª ØªØ±ÙŠØ¯ Ø¥ÙŠÙ‚Ø§ÙÙ‡ Ù…Ø¤Ù‚ØªØ§Ù‹ØŒ Ø§Ø³ØªØ®Ø¯Ù… 'ØªØ¬Ù…ÙŠØ¯' Ø¨Ø¯Ù„Ø§Ù‹ Ù…Ù† Ø§Ù„Ø­Ø°Ù.",
                reply_markup=keyboard,
                parse_mode="Markdown"
            )
            return UM_USER_ACTIONS
        
        await query.edit_message_text(f"{message}\n\nðŸ‘¥ Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†:", reply_markup=_main_kb(), parse_mode="Markdown")
        return UM_START
...
async def _show_suspended_users(query, context):
    ...
    with SessionLocal() as s:
        ...
        if not users:
            await query.message.reply_text(
                "ðŸ”“ **Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ù…Ø¬Ù…Ø¯ÙŠÙ†**\n\n"
        ...
        text = f"ðŸ”’ **Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ø§Ù„Ù…Ø¬Ù…Ø¯ÙŠÙ† ({len(users)})**\n\n"
        text += "Ø§Ø®ØªØ± Ù…Ø³ØªØ®Ø¯Ù…Ø§Ù‹ Ù„Ø¹Ø±Ø¶ Ø§Ù„ØªÙØ§ØµÙŠÙ„:"
        await query.edit_message_text(text, reply_markup=_users_kb(users), parse_mode="Markdown")
        return UM_SELECT_USER
...
async def _show_statistics(query, context):
    ...
    with SessionLocal() as s:
        ...
        text = f"ðŸ“Š **Ø¥Ø­ØµØ§Ø¦ÙŠØ§Øª Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†**\n"
        text += f"â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n\n"
        text += f"ðŸ‘¥ **Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†:** {total_users}\n"
        text += f"âœ… **Ø§Ù„Ù…Ø¹ØªÙ…Ø¯ÙŠÙ†:** {approved_users}\n"
        text += f"â³ **Ø§Ù„Ù…Ø¹Ù„Ù‚ÙŠÙ†:** {pending_users}\n"
        text += f"ðŸ”’ **Ø§Ù„Ù…Ø¬Ù…Ø¯ÙŠÙ†:** {suspended_users}\n"
        text += f"ðŸŸ¢ **Ø§Ù„Ù†Ø´Ø·ÙŠÙ†:** {active_users}\n\n"
        text += f"ðŸ“ **Ø¥Ø¬Ù…Ø§Ù„ÙŠ Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ±:** {total_reports}\n\n"
        
        if top_translators:
            text += f"ðŸ† **Ø£ÙƒØ«Ø± Ø§Ù„Ù…ØªØ±Ø¬Ù…ÙŠÙ† Ù†Ø´Ø§Ø·Ø§Ù‹:**\n\n"
...
async def handle_search_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as s:
        # Ø§Ù„Ø¨Ø­Ø« ÙÙŠ Ø§Ù„Ø§Ø³Ù… ÙˆØ§Ù„Ù‡Ø§ØªÙ
        ...
        if not users:
            await update.message.reply_text(
                f"âŒ **Ù„Ù… ÙŠØªÙ… Ø§Ù„Ø¹Ø«ÙˆØ± Ø¹Ù„Ù‰ Ù†ØªØ§Ø¦Ø¬**\n\n"
        ...
        text = f"ðŸ” **Ù†ØªØ§Ø¦Ø¬ Ø§Ù„Ø¨Ø­Ø« Ø¹Ù†:** `{search_text}`\n\n"
...
async def handle_delete_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_id:
        await query.edit_message_text("âŒ Ø®Ø·Ø£: Ù„Ù… ÙŠØªÙ… Ø§Ù„Ø¹Ø«ÙˆØ± Ø¹Ù„Ù‰ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù….", reply_markup=_main_kb())
        return UM_START
    ...
    with SessionLocal() as s:
        ...
        user.is_approved = False  # ØªØ£ÙƒØ¯ Ù…Ù† Ø¥Ù„ØºØ§Ø¡ Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø£ÙŠØ¶Ø§Ù‹
        user.is_suspended = False # ØªØ£ÙƒØ¯ Ù…Ù† Ø¥Ù„ØºØ§Ø¡ Ø§Ù„ØªØ¬Ù…ÙŠØ¯ Ø£ÙŠØ¶Ø§Ù‹
        s.commit()
    
    message = f"ðŸ—‘ **ØªÙ… Ø¥Ù„ØºØ§Ø¡ ØªÙ†Ø´ÙŠØ· Ø­Ø³Ø§Ø¨ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…**\n\n"
    message += f"ðŸ‘¤ **Ø§Ù„Ø§Ø³Ù…:** {user_name}\n"
    message += f"ðŸ“… **Ø§Ù„ØªØ§Ø±ÙŠØ®:** {datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
    message += f"âœ… **ØªÙ… Ø¨Ù†Ø¬Ø§Ø­:**\n"
    message += f"- Ø¥Ù„ØºØ§Ø¡ ØªÙ†Ø´ÙŠØ· Ø§Ù„Ø­Ø³Ø§Ø¨ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª\n"
    ...
    try:
        await context.bot.send_message(
            chat_id=user_tg_id,
            text=f"ðŸš« **ØªÙ… Ø¥Ù„ØºØ§Ø¡ ØªÙ†Ø´ÙŠØ· Ø­Ø³Ø§Ø¨Ùƒ**\n\n"
                 ...
                 f"Ù„Ù„Ù…Ø²ÙŠØ¯ Ù…Ù† Ø§Ù„Ù…Ø¹Ù„ÙˆÙ…Ø§Øª Ø£Ùˆ Ø§Ù„Ø§Ø³ØªÙØ³Ø§Ø±ØŒ ÙŠØ±Ø¬Ù‰ Ø§Ù„ØªÙˆØ§ØµÙ„ Ù…Ø¹ Ø§Ù„Ø¥Ø¯Ø§Ø±Ø©.",
            parse_mode="Markdown"
        )
        logger.info(f"âœ… ØªÙ… Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ø§Ù„Ø¥Ø®Ø±Ø§Ø¬ Ø¥Ù„Ù‰ {user_name}")
    except Exception as e:
        logger.error(f"âŒ ÙØ´Ù„ Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ø§Ù„Ø¥Ø®Ø±Ø§Ø¬: {e}")
...
async def handle_suspend_reason_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reasons = {
        "violation": "âš ï¸ Ù…Ø®Ø§Ù„ÙØ© Ù„Ù„Ù†Ø¸Ø§Ù… ÙˆØ§Ù„Ù„ÙˆØ§Ø¦Ø­",
        "performance": "ðŸ“‰ Ø£Ø¯Ø§Ø¡ Ø¶Ø¹ÙŠÙ ÙÙŠ Ø§Ù„Ø¹Ù…Ù„",
        "behavior": "ðŸš« Ø³Ù„ÙˆÙƒ ØºÙŠØ± Ù„Ø§Ø¦Ù‚ Ù…Ø¹ Ø§Ù„ÙØ±ÙŠÙ‚",
        "late": "â° ØªØ£Ø®Ø± Ù…ØªÙƒØ±Ø± ÙÙŠ ØªØ³Ù„ÙŠÙ… Ø§Ù„ØªÙ‚Ø§Ø±ÙŠØ±"
    }
    
    if reason_type == "custom":
        # Ø·Ù„Ø¨ Ø¥Ø¯Ø®Ø§Ù„ Ø³Ø¨Ø¨ Ù…Ø®ØµØµ
        await query.edit_message_text(
            "âœï¸ **Ø¥Ø¯Ø®Ø§Ù„ Ø³Ø¨Ø¨ Ø§Ù„ØªØ¬Ù…ÙŠØ¯**\n\n"
...
async def _execute_suspension(query, context, reason):
    ...
    message = f"ðŸ”’ **ØªÙ… ØªØ¬Ù…ÙŠØ¯ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… Ø¨Ù†Ø¬Ø§Ø­**\n\n"
    message += f"ðŸ‘¤ **Ø§Ù„Ø§Ø³Ù…:** {user_name}\n"
    message += f"ðŸ“‹ **Ø§Ù„Ø³Ø¨Ø¨:** {reason}\n"
    message += f"ðŸ“… **Ø§Ù„ØªØ§Ø±ÙŠØ®:** {datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
    ...
    try:
        await context.bot.send_message(
            chat_id=user_tg_id,
            text=f"ðŸ”’ **ØªÙ… ØªØ¬Ù…ÙŠØ¯ Ø­Ø³Ø§Ø¨Ùƒ**\n\n"
        ...
        logger.error(f"âŒ ÙØ´Ù„ Ø¥Ø±Ø³Ø§Ù„ Ø¥Ø´Ø¹Ø§Ø± Ø§Ù„ØªØ¬Ù…ÙŠØ¯: {e}")
    
    await query.edit_message_text(f"{message}\n\nðŸ‘¥ Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†:", reply_markup=_main_kb(), parse_mode="Markdown")


async def _execute_suspension_message(update, context, reason):
    """ØªÙ†ÙÙŠØ° Ø§Ù„ØªØ¬Ù…ÙŠØ¯ (Ù…Ù† Ø±Ø³Ø§Ù„Ø© Ù†ØµÙŠØ©)"""
    user_id = context.user_data.get("suspend_user_id")
    user_name = context.user_data.get("suspend_user_name")
    user_tg_id = context.user_data.get("suspend_user_tg_id")
    
    if not user_id:
        await update.message.reply_text("âŒ Ø®Ø·Ø£: Ù„Ù… ÙŠØªÙ… Ø§Ù„Ø¹Ø«ÙˆØ± Ø¹Ù„Ù‰ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù….", reply_markup=_main_kb())
        return
    
    with SessionLocal() as s:
        user = s.query(Translator).filter_by(id=user_id).first()
        if not user:
            await update.message.reply_text("âŒ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯.", reply_markup=_main_kb())
            return
...
async def cancel_user_management(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.clear()
    await update.message.reply_text("âŒ ØªÙ… Ø¥Ù„ØºØ§Ø¡ Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†.")
    return ConversationHandler.END
...
def register(app):
    ...
    conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex("^ðŸ‘¥ Ø¥Ø¯Ø§Ø±Ø© Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ†$"), start_user_management)
        ],
        states={
            UM_START: [
