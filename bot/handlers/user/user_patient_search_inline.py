...
async def patient_search_inline_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # إزالة "بحث: " من بداية النص إذا كان موجوداً
            ...
            try:
                ...
                for idx, patient in enumerate(patients):
                    ...
                    if patient.file_number:
                        description_parts.append(f"📄 رقم الملف: {patient.file_number}")
                    if patient.phone_number:
                        description_parts.append(f"📱 {patient.phone_number}")
                    if patient.age:
                        description_parts.append(f"🎂 {patient.age} سنة")
                    
                    description = " | ".join(description_parts) if description_parts else patient_name
        ...
        if not results:
            ...
            no_results = InlineQueryResultArticle(
                id="no_results",
                title="❌ لا توجد نتائج",
                description=f"لم يتم العثور على مرضى يحتوي اسمهم على '{query_text}'" if query_text else "لا توجد مرضى في قاعدة البيانات",
                input_message_content=InputTextMessageContent(
