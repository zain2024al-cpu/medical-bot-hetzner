# =============================
# Admission - Edit Before Publish Handlers
# =============================
# handlers منفصلة تماماً لمسار "ترقيد"
...
try:
    ...
    logger.error("❌ Cannot import required modules for admission_edit")
    ADMISSION_CONFIRM = None
...
async def handle_admission_edit_field_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ✅ معالجة اختيار حقل للتعديل - مسار ترقيد
    ...
    try:
        ...
        # Format: "edit_field:admission:field_key"
        ...
        logger.info(f"✏️ [ADMISSION] handle_edit_field_selection: field_key={field_key}")
        ...
        logger.info(f"✅ [ADMISSION] تم طلب تعديل حقل: {field_key}, state: {confirm_state}")
        return confirm_state
        
    except Exception as e:
        logger.error(f"❌ [ADMISSION] خطأ في handle_edit_field_selection: {e}", exc_info=True)
        try:
            query = update.callback_query
            await query.edit_message_text(
            ...
            logger.debug("تم تجاهل استثناء في handle_admission_edit_field_selection", exc_info=True)
        return ConversationHandler.END
...
async def handle_admission_edit_field_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ✅ معالجة إدخال القيمة الجديدة - مسار ترقيد
    منطق منفصل تماماً
    """
    try:
        ...
        # ✅ التحقق من أن هذا handler مخصص لمسار admission فقط
        if flow_type != "admission":
            logger.warning(f"⚠️ [ADMISSION] handle_edit_field_input: flow_type={flow_type} ليس admission - تجاهل")
            return
        
        if not field_key:
            logger.warning("⚠️ [ADMISSION] handle_edit_field_input: لا يوجد حقل للتعديل - تجاهل")
            return
        
        logger.info(f"✏️ [ADMISSION] handle_edit_field_input: field_key={field_key}, text={text[:50]}")
        ...
        if field_key == "followup_date":
            ...
            if "followup_time" not in data:
                data["followup_time"] = ""
        elif field_key == "admission_reason":
            data["admission_reason"] = text
        elif field_key == "room_number":
            data["room_number"] = text
        else:
            # للحقول الأخرى (notes, followup_reason, report_date, patient_name, hospital_name, department_name, doctor_name)
            data[field_key] = text
        ...
        logger.info(f"✅ [ADMISSION] تم حفظ current_flow={flow_type} في report_tmp")
        
        logger.info(f"✅ [ADMISSION] تم حفظ التعديل: {field_key} = {text[:50]}")
        ...
        try:
            ...
            confirm_state = get_confirm_state(flow_type)
            context.user_data['_conversation_state'] = confirm_state
            logger.info(f"✅ [ADMISSION] تم عرض الملخص بعد التعديل، flow_type={flow_type}, confirm_state={confirm_state}")
            return confirm_state
        except Exception as e:
            logger.error(f"❌ [ADMISSION] خطأ في عرض الملخص بعد التعديل: {e}", exc_info=True)
            await update.message.reply_text(
        ...
        logger.error(f"❌ [ADMISSION] خطأ في handle_edit_field_input: {e}", exc_info=True)
        try:
            await update.message.reply_text(
            ...
            logger.debug("تم تجاهل استثناء في handle_admission_edit_field_input", exc_info=True)
        return ConversationHandler.END
