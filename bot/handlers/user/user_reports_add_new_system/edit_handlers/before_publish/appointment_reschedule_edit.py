# =============================
# Appointment Reschedule - Edit Before Publish Handlers
# =============================
# handlers منفصلة تماماً لمسار "تأجيل موعد"
...
try:
    ...
    logger.error("❌ Cannot import required modules for appointment_reschedule_edit")
    APP_RESCHEDULE_CONFIRM = None
...
async def handle_appointment_reschedule_edit_field_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ✅ معالجة اختيار حقل للتعديل - مسار تأجيل موعد
    ...
    try:
        ...
        # Format: "edit_field:appointment_reschedule:field_key"
        ...
        logger.info(f"✏️ [APP_RESCHEDULE] handle_edit_field_selection: field_key={field_key}")
        ...
        logger.info(f"✅ [APP_RESCHEDULE] تم طلب تعديل حقل: {field_key}, state: {confirm_state}")
        return confirm_state
        
    except Exception as e:
        logger.error(f"❌ [APP_RESCHEDULE] خطأ في handle_edit_field_selection: {e}", exc_info=True)
        try:
            query = update.callback_query
            await query.edit_message_text(
...
async def handle_appointment_reschedule_edit_field_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ✅ معالجة إدخال القيمة الجديدة - مسار تأجيل موعد
    منطق منفصل تماماً
    """
    try:
        ...
        # ✅ التحقق من أن هذا handler مخصص لمسار appointment_reschedule فقط
        if flow_type != "appointment_reschedule":
            logger.warning(f"⚠️ [APP_RESCHEDULE] handle_edit_field_input: flow_type={flow_type} ليس appointment_reschedule - تجاهل")
            return
        
        if not field_key:
            logger.warning("⚠️ [APP_RESCHEDULE] handle_edit_field_input: لا يوجد حقل للتعديل - تجاهل")
            return
        
        logger.info(f"✏️ [APP_RESCHEDULE] handle_edit_field_input: field_key={field_key}, text={text[:50]}")
        ...
        if field_key == "followup_date":
            ...
            if " " in text:
                ...
                if "followup_time" not in data:
                    data["followup_time"] = ""
        elif field_key == "app_reschedule_return_date":
            # ✅ حفظ app_reschedule_return_date
            data["app_reschedule_return_date"] = text
            # ✅ أيضاً حفظه في followup_date للتوافق
            data["followup_date"] = text
        elif field_key == "app_reschedule_reason":
            data["app_reschedule_reason"] = text
        elif field_key == "app_reschedule_return_reason":
            data["app_reschedule_return_reason"] = text
            # ✅ أيضاً حفظه في followup_reason للتوافق
        ...
        logger.info(f"✅ [APP_RESCHEDULE] تم حفظ current_flow={flow_type} في report_tmp")
        
        logger.info(f"✅ [APP_RESCHEDULE] تم حفظ التعديل: {field_key} = {text[:50]}")
        ...
        try:
            ...
            confirm_state = get_confirm_state(flow_type)
            context.user_data['_conversation_state'] = confirm_state
            logger.info(f"✅ [APP_RESCHEDULE] تم عرض الملخص بعد التعديل، flow_type={flow_type}, confirm_state={confirm_state}")
            return confirm_state
        except Exception as e:
            logger.error(f"❌ [APP_RESCHEDULE] خطأ في عرض الملخص بعد التعديل: {e}", exc_info=True)
            await update.message.reply_text(
        ...
        logger.error(f"❌ [APP_RESCHEDULE] خطأ في handle_edit_field_input: {e}", exc_info=True)
        try:
            await update.message.reply_text(
