# =============================
# Rehab Physical - Edit Before Publish Handlers
# =============================
# handlers منفصلة تماماً لمسار "علاج طبيعي"
...
try:
    ...
    logger.error("❌ Cannot import required modules for rehab_physical_edit")
    PHYSICAL_THERAPY_CONFIRM = None
...
async def handle_rehab_physical_edit_field_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ✅ معالجة اختيار حقل للتعديل - مسار علاج طبيعي
    ...
    try:
        ...
        # Format: "edit_field:rehab_physical:field_key"
        ...
        field_key = parts[2]
        flow_type = "rehab_physical"
        
        logger.info(f"✏️ [REHAB_PHYSICAL] handle_edit_field_selection: field_key={field_key}")
        ...
        logger.info(f"✅ [REHAB_PHYSICAL] تم طلب تعديل حقل: {field_key}, state: {confirm_state}")
        return confirm_state
        
    except Exception as e:
        logger.error(f"❌ [REHAB_PHYSICAL] خطأ في handle_edit_field_selection: {e}", exc_info=True)
        try:
            query = update.callback_query
            await query.edit_message_text(
            ...
            logger.debug("تم تجاهل استثناء في handle_rehab_physical_edit_field_selection", exc_info=True)
        return ConversationHandler.END
...
async def handle_rehab_physical_edit_field_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    ✅ معالجة إدخال القيمة الجديدة - مسار علاج طبيعي
    منطق منفصل تماماً
    """
    try:
        ...
        # ✅ التحقق من أن هذا handler مخصص لمسار rehab_physical فقط
        if flow_type != "rehab_physical":
            logger.warning(f"⚠️ [REHAB_PHYSICAL] handle_edit_field_input: flow_type={flow_type} ليس rehab_physical - تجاهل")
            return
        
        if not field_key:
            logger.warning("⚠️ [REHAB_PHYSICAL] handle_edit_field_input: لا يوجد حقل للتعديل - تجاهل")
            return
        
        logger.info(f"✏️ [REHAB_PHYSICAL] handle_edit_field_input: field_key={field_key}, text={text[:50]}")
        ...
        if field_key == "followup_date":
            ...
            if " " in text:
                ...
                if "followup_time" not in data:
                    data["followup_time"] = ""
        elif field_key == "therapy_details":
            data["therapy_details"] = text
        else:
        ...
        logger.info(f"✅ [REHAB_PHYSICAL] تم حفظ current_flow={flow_type} في report_tmp")
        
        logger.info(f"✅ [REHAB_PHYSICAL] تم حفظ التعديل: {field_key} = {text[:50]}")
        ...
        try:
            ...
            confirm_state = get_confirm_state(flow_type)
            context.user_data['_conversation_state'] = confirm_state
            logger.info(f"✅ [REHAB_PHYSICAL] تم عرض الملخص بعد التعديل، flow_type={flow_type}, confirm_state={confirm_state}")
            return confirm_state
        except Exception as e:
            logger.error(f"❌ [REHAB_PHYSICAL] خطأ في عرض الملخص بعد التعديل: {e}", exc_info=True)
            await update.message.reply_text(
        ...
        logger.error(f"❌ [REHAB_PHYSICAL] خطأ في handle_edit_field_input: {e}", exc_info=True)
        try:
            await update.message.reply_text(
            ...
            logger.debug("تم تجاهل استثناء في handle_rehab_physical_edit_field_input", exc_info=True)
        return ConversationHandler.END
