...
# مسار غسيل الكلى (dialysis) — مبسَّط عن بقية أنواع جلسات العلاج: لا خطة
# (TreatmentPlan) ولا "عدد جلسات كلي" إطلاقاً، فقط رقم الجلسة الحالية
# يُطلَب يدوياً في كل تقرير (بلا تذكّر/اقتراح)، ثم رفع دفتر الجلسات
# (اختياري). شكوى المريض وقرار الطبيب (TREATMENT_COMPLAINT/TREATMENT_NOTES
# الموجودتان أصلاً) تُسألان أولاً قبل هاتين الحالتين. (131-132)
(
    TREATMENT_DIALYSIS_SESSION,  # رقم الجلسة الحالية (إدخال يدوي مباشر)
    TREATMENT_DIALYSIS_UPLOAD,   # رفع دفتر جلسات الغسيل / التقرير النهائي، أو تخطي
) = range(131, 133)
