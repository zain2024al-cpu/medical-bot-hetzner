...
# حالة إضافية: رقم الجلسة ضمن الدورة الحالية — العلاج الكيماوي فقط. كل
# دورة تحتوي عدة جلسات (TreatmentPlan.total_sessions/current_session
# لدى الكيماوي يمثّلان عدد/رقم **الدورة** لا الجلسة — انظر unit_labels في
# services/treatment_plan_service.py)، وهذا رقم يدوي إضافي منفصل تماماً
# لا خطة تتبعه ولا عدّاً كلياً لجلسات كل دورة (بطلب المستخدم صراحةً).
# يُسأل بعد "✅ متابعة" في شاشة عرض الخطة، قبل شكوى المريض.
(TREATMENT_CHEMO_SESSION_NUMBER,) = range(140, 141)

# حالة إضافية: نفس رقم الجلسة ضمن الدورة أعلاه، لكن داخل "طابور" الاختيار
# المتعدد لجلسات الأورام (مسار 15 أدناه) — الكيماوي هناك يمرّ بمعالِجات
# منفصلة تماماً (oncology_multiselect.py لا treatment_sessions.py)، فحالة
# treatment_sessions.py أعلاه لا تُستدعى إطلاقاً حين يُختار الكيماوي مع
# نوع آخر معاً. بلا هذه الحالة المنفصلة كان السؤال يختفي كلياً لأي مترجم
# يختار الكيماوي ضمن اختيار مُدمَج — وهو بالضبط ما أبلغ عنه المستخدم
# ("لم يظهر لبعض المترجمين").
(ONCOLOGY_QUEUE_CHEMO_SESSION,) = range(141, 142)

# مسار 15: اختيار متعدد لجلسات الأورام (كيماوي/مناعي/موجّه/إشعاعي معاً) —
# شاشة الاختيار (checkbox) نفسها تبقى ضمن حالة R_ACTION_TYPE (كبقية تصنيفات
# نوع الإجراء البصرية)، لكن معالجة كل نوع مُختار بدوره داخل الطابور تحتاج
# حالات فعلية لاستقبال نص. (127-130)
(
    ONCOLOGY_QUEUE_TOTAL,    # (داخل الطابور) كم عدد الجلسات الكلي؟ (مناعي/موجّه/إشعاعي فقط)
    ONCOLOGY_QUEUE_CURRENT,  # (داخل الطابور) رقم الجلسة الحالية؟
    ONCOLOGY_DELIVERY_MODE,  # طريقة الإعطاء: عيادة يومية / رقود
    ONCOLOGY_DELIVERY_DAYS,  # عدد أيام الرقود المتوقعة (إن اختير رقود)
) = range(127, 131)

# مسار غسيل الكلى (dialysis) — مبسَّط جداً عن بقية أنواع جلسات العلاج:
# بلا شكوى مريض، بلا قرار طبيب، بلا خطة (TreatmentPlan)، بلا رفع مرفقات.
# فقط: رقم الجلسة الحالية (يدوي، بلا تذكّر) ثم تاريخ الجلسة القادمة (تقويم
# مخصص، بلا وقت وبلا سبب) ثم المترجم مباشرة. (131-132)
(
    TREATMENT_DIALYSIS_SESSION,    # رقم الجلسة الحالية (إدخال يدوي مباشر)
    TREATMENT_DIALYSIS_NEXT_DATE,  # تاريخ الجلسة القادمة (تقويم، بلا وقت وبلا سبب)
) = range(131, 133)

# مسار 16: معاملة الزراعة (Transplant) — نوع إجراء جديد يظهر حصراً في قسم
# "🏙️ الرعاية الصحية - تشناي" (context.user_data["report_city"]=="chennai")،
# مخفي تماماً عن قائمة نوع الإجراء الاعتيادية. لا خطة علاجية ولا مريض
# طبي بالمعنى المعتاد — معاملة إدارية/قانونية مرتبطة بزراعة عضو.
# التسلسل: نوع الزراعة (كبد/كلى) → الجهة (اختيار متعدد) → تفاصيل
# المعاملة (نص حر) → تاريخ العودة → سبب العودة → المترجم → تأكيد.
# (133-139)
(
    TRANSPLANT_TYPE,            # نوع الزراعة: كبد / كلى (اختيار واحد)
    TRANSPLANT_PARTY,           # الجهة: المحكمة/المحامي/مكتب التنسيق (اختيار متعدد، ١ فأكثر)
    TRANSPLANT_DETAILS,         # تفاصيل المعاملة (نص حر يدوي)
    TRANSPLANT_FOLLOWUP_DATE,   # تاريخ العودة
    TRANSPLANT_FOLLOWUP_REASON, # سبب العودة
    TRANSPLANT_TRANSLATOR,      # اسم المترجم
    TRANSPLANT_CONFIRM,         # تأكيد
) = range(133, 140)