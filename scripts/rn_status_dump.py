...
def _nodates():
    """نشطة بلا تنبيه ولا انتهاء — **لا شيء يمكن أن يُحرّكها أبداً**.

    ⚠️ أخطر من المتأخّرة: المتأخّرة تنتقل متى عملت المهمة، أما هذه فليس
    لها تاريخ استحقاق إطلاقاً، فتبقى «نشطة» إلى الأبد ولا تظهر في أي
    قائمة معلّقات — بلا خطأ ولا تنبيه. مصدرها الشائع: شخص أُضيف بمزامنة
    مرافقي الوصول أو أُدخِل ولم تُستكمَل بياناته.
    """
    rows = q("SELECT id, name, status, COALESCE(reminder_date,''), "
             "COALESCE(expiry_date,'') FROM res_persons WHERE status='ACTIVE'")
    bad = [r for r in rows if not r[3].strip() and not r[4].strip()]
    if not bad:
        print("   ✅ كل الحالات النشطة لها تاريخ استحقاق.")
        return
    print(f"   🟠 {len(bad)} حالة نشطة بلا أي تاريخ — لن تنتقل تلقائياً أبداً:")
    for pid, nm, _st, _r, _e in bad:
        print(f"      #{pid:<4} {str(nm)[:34]}")
    print("      ⇐ تُعالَج بـ«✏️ تعديل البيانات» بإدخال تاريخ الانتهاء والتنبيه.")


section("📊 الحالات كما هي مخزَّنة حرفياً:", _statuses)
def _noremind():
    """لها انتهاء بلا تنبيه — تنتقل **يوم انتهائها** لا قبله.

    ⚠️ فئة لا يراها فحص «بلا أي تاريخ» لأنه يشترط خلوّ الحقلين. مصدرها
    الأساسي: `confirm_issuance` كان يُصفّر التنبيه عمداً عند توثيق كل
    تمديد (أُصلِح ٢٠٢٦-٠٩-٠٣)، فكل من وُثِّق تمديده قبل ذلك التاريخ صار
    نشطاً بلا تنبيه — ينتقل إلى «معلّق انتهاء» يوم انتهاء إقامته، أي بعد
    فوات الغرض من التنبيه.
    """
    rows = q("SELECT id, name, COALESCE(reminder_date,''), COALESCE(expiry_date,'') "
             "FROM res_persons WHERE status='ACTIVE'")
    bad = [r for r in rows if not r[2].strip() and r[3].strip()]
    if not bad:
        print("   ✅ كل الحالات النشطة لها تاريخ تنبيه.")
        return
    print(f"   🟠 {len(bad)} حالة نشطة بتاريخ انتهاء بلا تنبيه:")
    for pid, nm, _r, exp in sorted(bad, key=lambda x: x[3]):
        print(f"      #{pid:<4} {str(nm)[:30]:<32} تنتهي {exp}")
    print("      ⇐ افتح الحالة ← «✏️ تعديل البيانات» ← «تاريخ التنبيه».")
    print("      (المدة قبل الانتهاء يحدّدها المستخدم — لا يضبطها النظام تلقائياً.)")


section("🕳 نشطة بلا أي تاريخ (لن تنتقل أبداً):", _nodates)
section("🔔 نشطة بلا تنبيه (تنتقل يوم انتهائها لا قبله):", _noremind)
