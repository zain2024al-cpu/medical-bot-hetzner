...
class Report(Base):
    ...
    # ✅ **المحادثة** التي نُشِرت فيها البطاقة — معرّفات الرسائل في
    # تيليجرام خاصة بكل محادثة، فبلا هذا العمود يبقى
    # `group_message_id` وحده بلا معنى: كان الكود يخمّن المحادثة من
    # `REPORTS_GROUP_ID` وهي قد تحمل **أكثر من معرّف مفصولة بفواصل**
    # ("-100…388,-100…845") فلا تصلح كـchat_id أصلاً، أو تخصّ مجموعة
    # أخرى تماماً (تشناي). يُملأ وقت النشر ويُستعمل حرفياً عند
    # التعديل/الحذف. فارغ للتقارير القديمة ⇒ يُشتَقّ كما سابقاً.
    ...
    # ✅ رقم الجلسة ضمن الدورة الحالية — العلاج الكيماوي فقط. كل دورة
    # كيماوي تحتوي عدة جلسات، وTreatmentPlan.current_session يتتبَّع رقم
    # **الدورة** لا الجلسة (انظر تعليق treatment_plan_summary أعلاه)، فهذا
    # عمود مستقل تماماً: رقم يدوي بلا خطة تتبعه وبلا عدّ كلي لجلسات كل
    # دورة (بطلب المستخدم صراحةً — فقط "كم رقم هذه الجلسة؟" في كل تقرير).
    chemo_session_number = Column(Integer, nullable=True)
...
class ResidencyPerson(Base):
    ...
    # ✅ تاريخ **آخر إصدار** للإقامة — يُدخَل يدوياً عند معالجة "🏠 معلّقات
    # من الحالات السابقة" (المريض القديم قد يملك إقامة سارية أُصدرت قبل
    # تفعيل الوحدة، فلا سجلّ إصدار لها في النظام). فارغ لمن لم يُدخَل له.
    last_issue_date    = Column(String(50),  default="")
